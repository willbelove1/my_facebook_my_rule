 ...
 * Phiên bản: 2.0.0 (Enhanced with Light Mode & Toggle All)
 */
(function() {
  'use strict';
  ...
  window.FBCMF.registerModule('UIManager', async (ctx) => {
    ...
    // Làm sạch nội dung để tránh lỗi
    const cleanContent = (content) => {
      return content.replace(/[-\u001F\u007F-\u009F]/g, '').trim();
    };
    
    // Tạo CSS cho cả hai theme
    const createStyles = () => {
      const darkTheme = `
        #fbcmf-clean-btn {
    ...
    const initUI = async () => {
      ...
      try {
        ...
        popup.innerHTML = cleanContent(popupContent);
